...
var Channel = /** @class */ (function (_super) {
    ...
    Channel.prototype.addUser = function (user) {
        ...
        if (this._channelAssigns[clientId]) {
            throw new basePromise_1.PondError('User already exists in channel', 5001, clientId);
        }
        var doc = this._channelPresence.set(user.presence);
        ...
        this._broadcast.publish({
            event: 'JOIN_CHANNEL', channelName: this.name, action: enums_1.ServerActions.PRESENCE, payload: {
                presence: this.presence, change: user.presence
            }, channel: this, clientId: clientId, clientAssigns: user.assigns, clientPresence: user.presence
        });
    };
    /**
        ...
        clients.forEach(function (clientId) {
            ...
            if (client) {
                ...
                _this._broadcast.publish({
                    event: 'LEAVE_CHANNEL', channelName: _this.name, action: enums_1.ServerActions.PRESENCE, payload: {
                        presence: _this.presence, change: null,
                    }, channel: _this, clientId: clientId, clientAssigns: client.assigns, clientPresence: client.presence
                });
            }
        });
    ...
    Channel.prototype.updateUser = function (clientId, presence, assigns) {
        ...
        if (client) {
            ...
            var presenceDoc = __assign(__assign({}, client.presence.doc), presence);
