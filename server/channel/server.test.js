...
describe('server', function () {
    it('should be defined', function () {
        expect(server_1.Server).toBeDefined();
    });
    it('should be instantiable', function () {
        expect(new server_1.Server()).toBeInstanceOf(server_1.Server);
    });
    it('should ba able to create its own server and websocket server if none is provided', function () {
        var socket = new server_1.Server();
    ...
    it('should take a server and websocket server if provided', function () {
        ...
        var socket = new server_1.Server(server, socketServer);
    ...
    it('should throw an error when the server throws an error', function () {
        var server = require('http').createServer();
        var socket = new server_1.Server(server);
        socket.listen(3001, function () { });
        expect(function () { return server.emit('error', new Error('test')); }).toThrowError(basePromise_1.PondError);
        server.close();
    });
