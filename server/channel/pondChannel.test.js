        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    ...
                    doc.removeDoc();
                    expect(function () { return pondChannel.getChannelInfo('/test'); }).toThrowError(basePromise_1.PondError); // channel is removed from the pond channel and can not be found
                ...
                case 4:
                    error_1 = _a.sent();
                    expect(error_1).toBeInstanceOf(basePromise_1.PondError); // this is because, the pondChannel handler doesn't act on the response
                    expect(error_1).toEqual(new basePromise_1.PondError('Function did not resolve a Promise', 500, {
                        channelName: '/test',
                    }));
                    return [3 /*break*/, 5];
                case 5:
                ...
                case 9:
                    error_2 = _a.sent();
                    expect(error_2).toBeInstanceOf(basePromise_1.PondError); // this is because we reject the connection
                    expect(error_2).toEqual(new basePromise_1.PondError('test', 69420, {
                ...
                case 12:
                    error_3 = _a.sent();
                    expect(error_3).toBeInstanceOf(basePromise_1.PondError); // this is because we reject the connection
                    expect(error_3).toEqual(new basePromise_1.PondError('Message rejected', 403, {
                        channelName: '/balls',
                    }));
                    return [3 /*break*/, 13];
                case 13:
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                ...
                case 2:
                    ...
                    expect(encompassingMessageCount).toBe(4); // 1 for each join event, 1 for each fallback event
                    expect(userMessageCount).toBe(5); // first user gets their join event and the seconds join, the second receives only theirs
                    // for each presence change, the fallback event is called once and the message emitted by the fallback event is sent to the user who emitted the presence event
                    encompassingMessageCount = 0;
                    userMessageCount = 0;
                    try {
                    ...
                    expect(narrowedMessageCount).toBe(1); // the send event is caught by the narrowed handler
                    expect(encompassingMessageCount).toBe(2); // the (send) message modifies the user presence, which is caught by the encompassing handler
                    // The encompassing handler sends a message to the user whose presence was changed (test2)
                    // This message is then caught by the encompassing handler and when it tries to send to the sender which is itself, it is rejected
                    expect(userMessageCount).toBe(4); // the message is sent since it was accepted in the handler
                    // the user's presence is modified and sent to all users
                    // the user whose presence was modified receives a message from the encompassing handler
